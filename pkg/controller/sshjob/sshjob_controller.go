...
func (r *ReconcileSSHJob) Reconcile(request reconcile.Request) (reconcile.Result, error) {
  ...
  podShouldExist := len(annotationIn) > 0
  podExists := err == nil || !errors.IsNotFound(err)

  switch {
  case podExists && podShouldExist:
  ...
  case podExists && !podShouldExist:
    ...
    // Delete pod
    reqLogger.Info("Deleting a Pod", "Pod.Namespace", found.Namespace, "Pod.Name", found.Name, "Annotation", annotationIn)
